...
const dispatchTrip = catchAsync(async (req, res) => {
  const trip = await tripService.updateTripStatus(req.params.id, 'IN_PROGRESS', 'Trip dispatched via legacy route', req.user.id);
  sendSuccess(res, 200, 'Trip dispatched successfully', trip);
});

const completeTrip = catchAsync(async (req, res) => {
  const { actualDistance, fuelConsumed, actualFuel } = req.body;
  const updateData = {};
  if (actualDistance !== undefined) updateData.actualDistance = actualDistance;
  if (fuelConsumed !== undefined) updateData.actualFuel = fuelConsumed;
  if (actualFuel !== undefined) updateData.actualFuel = actualFuel;
  
  if (Object.keys(updateData).length > 0) {
    await tripService.updateTrip(req.params.id, updateData, req.user.id);
  }
  const trip = await tripService.updateTripStatus(req.params.id, 'COMPLETED', 'Trip completed via legacy route', req.user.id);
  sendSuccess(res, 200, 'Trip completed successfully', trip);
});

const cancelTrip = catchAsync(async (req, res) => {
  const trip = await tripService.updateTripStatus(req.params.id, 'CANCELLED', 'Trip cancelled via legacy route', req.user.id);
